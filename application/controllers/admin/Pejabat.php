...
$this->db->join("departements", "departements.id = pejabats.departement_id", "left");        $this->datatables->from('pejabats');
        ...
        $btn_edit = false;
        if ($this->is_allowed('pejabat_update', false) == true) {
        	$btn_edit = '
        		        	<a title="Edit data terpilih" class="btn btn-xs btn-warning" href="'. site_url('admin/pejabat/edit/$1').'">
        		            <i class="fa fa-pencil"></i>
        		          </a>';
        }
        $btn_delete = false;
        if ($this->is_allowed('pejabat_delete', false) == true) {
		...
		if ($this->is_allowed('pejabat_benned', false) == true) {
      		$status = true;
      	}		        	                  
