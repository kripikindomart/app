...
if (!function_exists('getDropdownList')) {
	function getDropdownList($table, $columns, $condition = false)
    {
        $ci =& get_instance();
        if ($condition != false) {
...
if (!function_exists('get_geolocation')) {
    ...
    function get_geolocation($ip) {
		...
		//$details = json_decode(file_get_contents("localhost"));
...
if (!function_exists('get_lang_by_ip')) {
    ...
    function get_lang_by_ip($ip) {
    	...
    	//$location = get_geolocation('127.0.0.1');
    ...
    function tampil_media($file,$width="100%",$height="100%") {
        ...
        if (!in_array($eks, $eks_video) && !in_array($eks, $eks_audio) && !in_array($eks, $eks_image)) {
            ...
            if (in_array($eks, $eks_image)) {
                if (is_file("./".$file)) {
                    $img_url = BASE_URL.$file;
                     $ret .= ' <a class="fancybox" rel="group" href=" '.$img_url.'">
                                          <img class="thumbnail w-100" src="'.base_url().$file.'" style="width: '.$width.'; height: '.$height.';" />
                                         </a>';
