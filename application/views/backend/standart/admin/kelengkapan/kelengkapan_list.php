               ...
               <div class="box box-widget widget-user-2">
                  ...
                  <div class="widget-user-header ">
                     <div class="row pull-right">
                        <?php //is_allowed('angkatan_add', function(){?>
                        <button class="btn btn-flat btn-success btn_add_new" id="add" title="<?= cclang('add_new_button', 'Kelengkapan'); ?> (Ctrl+a)" href="<?= site_url('admin/angkatan/add'); ?>"><i class="fa fa-plus-square-o" ></i> <?= cclang('add_new_button', 'Kelengkapan'); ?></button>
                        <?php //}) ?>
                     ...
                     <!-- /.widget-user-image -->
                     <h3 class="widget-user-username">Angkatan</h3>
                     <h5 class="widget-user-desc"><?= cclang('list_all', 'Angkatan'); ?> <i class="label bg-yellow count-label">  <?= cclang('items'); ?></i></h5>
                  </div>

                  <form name="form_group" id="form_group" action="<?= base_url('admin/angkatan/index'); ?>">
                ...
                <form class="form form-horizontal" method="post" enctype="multipart/form-data" id="form">
                  <input type="hidden" name="id">
                    <div class="form-group ">
                        <label for="label" class="col-sm-2 control-label">Tahun Angkatan</label>
                        <div class="col-sm-8">
                          <input type="text" class="form-control" name="tahun_angkatan" id="tahun_angkatan" placeholder="Tahun Angkatan" value="">
                  ...
                  <input type="hidden" name="id">
                    <div class="form-group ">
                        <label for="label" class="col-sm-2 control-label">Aktif</label>
                        <div class="col-sm-8">
...
<script type="text/javascript" src="<?= APP.'angkatan/angkatan_list.js' ?>"></script>
<!-- Page script -->
<script>
    HTTP_REFERER = '<?= base_url('admin/angkatan') ?>'
  ...
  $(document).ready(function() {
    ...
    */
    $(document).on('click', '.edit', function(){
      $('#modal_form').modal('show'); 
        ...
        save_method = 'edit';
        $.ajax({
            url: '<?= base_url()?>admin/angkatan/edit/',
            type: 'POST',
            dataType: 'json',
            data: {id:id},
        }) 
        ...
        .done(function(res){
            if (res.success) {
                ...
                $('[name=tahun_angkatan]').val(res.message.keterangan);
                $('[name=aktif]').val(res.message.aktif);
                $('.btn_save_back').hide();
                $('#btnSave').text('Update Data');
                $('.title-box').text('Edit Tahun Angkatan'); // Set Title to Bootstrap modal title  
    ...
    */
    ...
    $('.btn_action').click(function(event) {
          if (save_method == 'add') {
               var url = '<?= base_url('admin/angkatan/save'); ?>';
                var save_type = $(this).attr('data-stype');
          } else {
              var url = '<?= base_url('admin/angkatan/edit_save'); ?>';
              var save_type = 'back';
          }
           $.ajax({
             url: url,
       ...
       var delete_id = $(this).attr('data-id');
        swal({
            ...
            function(isConfirm) {
                if (isConfirm) {
                   $.ajax({
                        url :'<?= base_url()?>admin/angkatan/delete',
                        type :'POST',
                        dataType: 'json',
