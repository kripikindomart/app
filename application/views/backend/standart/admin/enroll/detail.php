    ...
    <div class="col-md-12">
      ...
      <div class="box box-info">
        <div class="box-header with-border">
          <div class="row">
            <div class="col-sm-4">
              <a href="<?= site_url('admin/enroll/create/').$this->uri->segment(4) ?>" class="btn btn-sm btn-info btn-flat " id="add">Tambah data</a>
              
              <button type="button" onclick="reload_ajax()" class="btn btn-sm btn-flat btn-default"><i class="fa fa-refresh"></i> Reload</button>

              <a href="<?= site_url('admin/enroll/') ?>" class="btn btn-sm btn-success btn-flat " id="add">Kembali</a>
        ...
        <!-- /.box-header -->
        <div class="box-body">
        <?= form_open('admin/enroll/delete', array('id' => 'form_user')); ?>
...
<script >
  ...
  $('#prodi_filter').on('change', function(){
    ...
    let src = '<?=base_url()?>admin/enroll/ajax';
    let url;
  ...
  table = $("#soal").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: BASE_URL + "admin/enroll/detail_ajax/" + "<?= $this->uri->segment(4)?>",
      type: "POST"
      //data: csrf
    },
    columns: [
    ...
    columnDefs: [
      ...
      },
      {
        searchable: false,
        targets: 6,
        data: {id : 'id', id_group : 'id_group'},
        render: function(data, type, row, meta) {
          return `<div class="text-center">
                                <a href="${BASE_URL}admin/enroll/detail_soal/`+'<?= $this->uri->segment(4) ?>'+`/${data.id_group}"  class="btn btn-xs btn-success ">
                                    <i class="fa fa-search"></i> Detail Soal
                                </a>
                                <button type="button" class="btn btn-xs btn-danger delete" data-id="${data.id}">
                                    <i class="fa fa-trash"></i> Unenroll
                                </button>
                            </div>`;
        }
      }
  ...
  $(".select_all").on("click", function() {
   ...
   $('.apply').click(function() {
        ...
        if (bulk.val() == 'delete') {
             swal({
             title: "Anda Yakin ?",
             text: "Data akan di Unenroll dari Prodi, Anda bisa meng enroll kembali kelas di menu group soal !",
             type: "warning",
             ...
             closeOnCancel: true
                 ...
                 function(isConfirm) {
                     if (isConfirm) {
                       $.ajax({
                            url :'<?= base_url()?>admin/enroll/delete',
                            type :'POST',
                            dataType: 'json',
                            data: serialize_bulk, 
                        })
