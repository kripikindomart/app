...
class Aauth {
	...
	public function login($identifier, $pass, $remember = false, $totp_code = null) {
		...
		if($this->config_vars['ddos_protection'] && $this->config_vars['recaptcha_active'] && $this->get_login_attempts() > $this->config_vars['recaptcha_login_attempts']){
 		...
 		if( $this->config_vars['login_with_name'] == true && filter_var($identifier, FILTER_VALIDATE_EMAIL) == true){
 		...
 		}else if($this->config_vars['login_with_name'] === true){
 		...
 		// 	if (filter_var($identifier, FILTER_VALIDATE_EMAIL) === true) {
 		// 		$this->CI->load->helper('email');
	 	// 		if( !valid_email($identifier) OR strlen($pass) < $this->config_vars['min'] OR strlen($pass) > $this->config_vars['max'] )
			// 	{
			// 		$this->error($this->CI->lang->line('aauth_error_login_failed_email'));
		...
		if ($query->num_rows() != 0) {
			...
			if ($this->verify_password($password, $row->pass)) {
				...
				$this->error('Password tidak benar');
				return false;
			}
		}
		// if not matches
	 ...
	 * @return bool Add success/failure
	 */
	public function add_departement($user_id, $group_par) {
		...
		$query = $this->aauth_db->where('id_users',$user_id);
		$query = $this->aauth_db->where('id_departement',$group_id);
		$query = $this->aauth_db->get($this->config_vars['user_to_departement']);

		if ($query->num_rows() < 1) {
			$data = array(
				'id_users' => $user_id,
				'id_departement' => $group_id
			);

			return $this->aauth_db->insert($this->config_vars['user_to_departement'], $data);
		}
		$this->info($this->CI->lang->line('aauth_info_already_member'));
		return true;
	}
	...
	public function print_errors_landing($divider = '')
	{
		//$this->error( 'No Pendaftaran tidak terdaftar / Harap hubungi Panitia terlebihdahulu untuk mengetahu no pendaftaran' );

		$msg = 'No Pendaftaran tidak terdaftar / Harap hubungi Panitia terlebihdahulu untuk mengetahu no pendaftaran';
		// $msg_num = count($this->errors);
		// $i = 1;
		// foreach ($this->errors as $e)
		// {
